...
# ------------------------------------------------------------------
# Following Codes come from  https://github.com/setsunil/DSDNet.
# Date: Jun 24, 2023.
# ------------------------------------------------------------------
...
def fft_deconv(y, k):
    # Modifiled by sjkong, 2023/06/24
    # version: PyTorch <= 1.7.1
    p = padlrtb(k)
    k = torch.rot90(k, 2, (-1, -2))
    y = edgetaper_torch(reppad(y, k), k)
    Gx = p2o(torch.tensor([[-1, 1]]).type_as(y).unsqueeze(0).unsqueeze(0), y.shape[2:])
    Gy = p2o(torch.tensor([[-1], [1]]).type_as(y).unsqueeze(0).unsqueeze(0), y.shape[2:])
    F = p2o(k.unsqueeze(1), y.shape[2:])
    # A = torch.abs(F).pow(2) + 0.002*(torch.abs(Gx).pow(2)+torch.abs(Gy).pow(2))
    A = r2c(cabs2(F)) + 0.002 * (r2c(cabs2(Gx)) + r2c(cabs2(Gy)))
    # b = F.conj()* PF.fft2(y)
    Fc = cconj(F, inplace=False)
    b = cmul(Fc, rfft(y))  # F(k)_bar * F(y)
    # return PF.ifft2(b/A).real[...,p[0]:-p[1],p[2]:-p[3]]
    return irfft(cdiv(b, A))[..., p[0]:-p[1], p[2]:-p[3]]
...
def fft_conv(img, kernel):
    ...
    p = padlrtb(kernel)
    # X = PF.fft2(img)
    X = torch.rfft(img, 2, normalized=False, onesided=False)
    K = p2o(kernel, img.shape[2:])
    # return PF.ifft2(X*K).real[...,p[0]:-p[1],p[2]:-p[3]]
    return torch.irfft(cmul(X, K), 2, onesided=False)[..., p[0]:-p[1], p[2]:-p[3]]
...
def edgetaper_alpha_torch(kernel, img_shape):
    ...
    v = []
    for i in range(1,3):
        z = torch.fft.fft(torch.sum(kernel,-i),img_shape[i+1]-1)
        z = torch.fft.ifft(torch.square(torch.abs(z))).real
